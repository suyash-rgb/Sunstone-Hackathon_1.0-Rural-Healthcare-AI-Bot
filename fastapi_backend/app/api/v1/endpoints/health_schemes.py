...
from app.services.health_schemes_service import HealthSchemesService
...
async def get_health_schemes(
    ...
    return await HealthSchemesService.get_schemes_paginated(
...
async def get_available_states(db: AsyncSession = Depends(get_session)):
    ...
    return await HealthSchemesService.get_available_states(db=db)
...
async def get_available_categories(db: AsyncSession = Depends(get_session)):
    ...
    return await HealthSchemesService.get_available_categories(db=db)
...
async def get_scheme_detail(id_or_slug: str, db: AsyncSession = Depends(get_session)):
    ...
    scheme = await HealthSchemesService.get_scheme_by_id_or_slug(db=db, id_or_slug=id_or_slug)
    if not scheme:
...
async def rag_hybrid_search(
    ...
    return await HealthSchemesService.perform_rag_hybrid_search(
